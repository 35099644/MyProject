		...
		createNew:function(){
			...
			pclient.request={
                ...
                'send':function(sid,vid,routeid,touid,method,val){
                    var fuid = pclient.request.uuid;
                    var tag=2;
                    var kind=Kind.STRING;
                    pclient.request.msgbody.send(tag,kind,"#"+sid+":"+vid+":"+routeid+":"+fuid+":"+touid+":"+method+":#"+val);
                },
                'sendbytes':function(sid,vid,routeid,touid,method,type,msg,sync){
                    if(sync){
                       if(byte_cache){
                            alert("正在同步上一次未发送完的二进制数据!");
                            return;
                       }else{
                            byte_cache={};
                       }
                    ...
                    if(page==1){
                        ...
                        var pstr = "#"+sid+":"+vid+":"+routeid+":"+fuid+":"+touid+":"+method+":"+type+":"+"0"+":"+sn+":#";
                        ...
                        for(var i=0;i<page;i++){
                            ...
                            var uint_head = [tag,kind];
                            var pstr = "#"+sid+":"+vid+":"+routeid+":"+fuid+":"+touid+":"+method+":"+type+":"+i+":"+sn+":#";
                    ...
                    var uint_head = [tag,kind];
                    var pstr = "#"+sid+":"+vid+":"+routeid+":"+fuid+":"+touid+":"+method+":"+type+":"+page+":"+sn+":";
                    for(var j=0;j<pstr.length;j++){
                        uint_head[uint_head.length]=pstr.charCodeAt(j);
                    }
                    var head = new Uint8Array(uint_head);
